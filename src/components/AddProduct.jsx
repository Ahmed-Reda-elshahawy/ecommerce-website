...
import { HideAdd } from '../rtk/slices/AddSlice';
import { AddNewProduct } from '../rtk/slices/productsSlice';
...
const AddProduct = () => {
    let addMOdal = useSelector(state => state.AddModal);
    ...
    const handleAdd = (newProduct) => {
        dispatsh(AddNewProduct(newProduct));
        dispatsh(HideAdd());
        fireAlert();
    }
    ...
    return (
        <>
            <Modal show={addMOdal.show} onHide={() => dispatsh(HideAdd())}>
                ...
                <Modal.Body>
                    <form className="form login-form mt-1 mb-1" onSubmit={submitFun}>
                        <div className="input d-flex flex-column mb-3">
                            ...
                            <input type="number" name="id" min='0' id="product-id" className='border-0 border-bottom p-1 pt-1' {...register("id", { required: "This is required" })} onChange={(e) => setId(e.target.value)} />
                            {errors.id?.message && <p className='m-0' style={{ color: "red", fontSize: "0.7rem" }}>{errors.id?.message}</p>}
                        </div>
                        <div className="input d-flex flex-column mb-3">
                        ...
                        <button type='submit' className='btn btn-primary d-block mx-auto mt-3'>Add Product</button>
                    </form>
                </Modal.Body>
            </Modal>
        </>
